<script lang="ts">
  ...
  import {
    ...
    getRandomBatch,
    getImagesByBatch,
    getRandomImagesFromBatch,
    type GalleryItem,
  } from "$lib/data/fightingAI";
...
>
    ...
    <div class="flex justify-center items-center">
      ...
      <div class="relative mx-3">
        <h2 class="font-paplane text-4xl md:text-5xl text-primary">
          <span class="border-2 border-primary px-5 rounded-[20px] inline-block"
            >WITH AI</span
          >
        </h2>
      </div>
  ...
  <div
    class="images-gallery-wrapper relative w-full max-w-6xl mx-auto mb-8 px-4"
  >
    <div class="images-gallery relative h-[600px]">
      {#each batch as item, i (item.id)}
        ...
        >
          <div
            class="absolute inset-0 bg-primary opacity-0 hover:opacity-20 transition-opacity duration-300 ease-in-out z-10"
          ></div>
          <img
    ...
    >
      ...
      >
        ...
        <div class="flex flex-col md:flex-row md:h-[650px] max-h-[90vh]">
          <!-- Image -->
          <div
            class="img-container w-full md:w-1/2 p-6 flex items-center justify-center overflow-hidden"
          >
            <div
              class="relative overflow-hidden rounded-xl shadow-2xl w-full h-full max-h-[500px] md:max-h-full"
            >
              <img
              ...
              />
              <div
                class="absolute inset-0 shadow-inner pointer-events-none border border-white/10 rounded-xl"
              ></div>
            </div>
          </div>
          ...
          <!-- Content -->
          <div
            class="slide-content w-full md:w-1/2 p-8 bg-[#FFF9EB] overflow-y-auto custom-scrollbar"
          >
            <div class="mb-6">
              <div
                class="batch-label text-xs mb-2 uppercase tracking-wider inline-block border-2 border-primary rounded-xl px-4 py-1 text-primary font-paplane font-bold"
              >
                {currentItem.batch === 1
                  ? "Viral Social Media"
                  : currentItem.batch === 2
              ...
              <h3
                class="slider__image-title text-4xl font-bold text-primary mb-4 font-paplane uppercase leading-tight"
              >
                {currentItem.title}
              </h3>
              ...
              <p
                class="text-gray-800 mb-8 leading-relaxed font-paplane text-lg"
              >
            ...
            {#if currentItem.sourceAttribution}
              <div
                class="source-attribution bg-white/40 rounded-xl p-4 shadow-sm mb-6"
              >
                <h4
                  class="slider__person-label text-lg font-semibold mb-2 flex items-center font-paplane"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="ml-7">
                  <span
                    class="slider__person-name font-paplane font-bold text-primary text-xl block"
                    >{currentItem.sourceAttribution}</span
                  >
                  {#if currentItem.platform}
                    <span
                      class="text-sm text-gray-600 font-paplane flex items-center mt-1"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
            ...
            {#if currentItem.stats}
              <div
                class="viral-stats-container bg-white/40 rounded-xl p-4 shadow-sm mb-6"
              >
                <h4
                  class="slider__context-label text-lg font-semibold mb-3 font-paplane flex items-center"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-5 w-5 mr-2 text-primary"
                ...
                <ul
                  class="viral-stats grid grid-cols-1 md:grid-cols-3 gap-3 font-paplane ml-7"
                >
                  {#if currentItem.stats.likes}
                    <li
                      class="stat-card bg-white rounded-lg p-3 shadow-sm flex items-center transition-transform hover:translate-y-[-2px]"
                    >
                      <div
                        class="stat-icon bg-primary/10 rounded-full w-10 h-10 flex items-center justify-center mr-3"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                      ...
                      <div>
                        <span
                          class="text-primary font-medium text-xs uppercase block"
                          >Likes</span
                        >
                        <span class="text-gray-900 font-bold text-lg"
                          >{formatNumber(currentItem.stats.likes)}</span
                        >
                      </div>
                    </li>
                  {/if}
                  {#if currentItem.stats.shares}
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                        <span class="text-gray-900 font-bold text-lg"
                          >{formatNumber(currentItem.stats.shares)}</span
                        >
                      </div>
                    </li>
                  {/if}
                  {#if currentItem.stats.views}
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                        <span class="text-gray-900 font-bold text-lg"
                          >{formatNumber(currentItem.stats.views)}</span
                        >
                      </div>
                    </li>
                  {/if}
