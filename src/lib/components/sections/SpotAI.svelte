<script lang="ts">
  ...
  const totalGroups = $derived(() =>
    Math.ceil(carouselItems.length / cardsPerGroup)
  );
  const groupStart = $derived(() => currentGroup * cardsPerGroup);
  const groupItems = $derived(() =>
    carouselItems.slice(groupStart(), groupStart() + cardsPerGroup)
  );
...
<section
  id="spotAI"
  class="relative w-full min-h-screen flex flex-col items-center justify-center overflow-hidden bg-[var(--color-primary)] py-16 md:py-24"
>
  ...
  <div class="absolute inset-0 pointer-events-none z-10">
    <div class="opacity-70">
      <DigitalElements type="dot" pos="top-[10%] left-[15%]" color="white" />
    </div>
    <div class="opacity-70">
      <DigitalElements
        type="dot"
    ...
    <div class="opacity-50">
      <DigitalElements type="arrow" pos="top-[20%] right-[15%]" color="white" />
    </div>
    <div class="opacity-50">
      <DigitalElements
        type="arrow"
  ...
  <div
    class="relative z-20 flex flex-col items-center w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8"
  >
    <div class="mb-16">
      <h2
        class="font-paplane text-6xl md:text-7xl font-extrabold uppercase text-center text-white tracking-[0.22em]"
      >
        HOW TO SPOT AI
      </h2>
    ...
    <div class="relative flex flex-col items-center w-full max-w-5xl mx-auto">
      ...
      <div
        class="slider_arrows white mt-0 hidden md:flex justify-between absolute w-full top-1/2 -translate-y-1/2 px-4 z-20"
      >
      ...
      >
        ...
        <div
          class="flex flex-row justify-start md:justify-center items-stretch gap-4 w-full overflow-x-auto md:overflow-visible pb-4 md:pb-0 snap-x snap-mandatory md:snap-none scrollbar-hide"
        >
          {#each groupItemsArr() as item, i}
            <div
              class="snap-center w-[175px] sm:w-[200px] md:w-[220px] flex-shrink-0"
            >
              <SpotAICard
                title={item.title}
                ...
                image={{
                  ...
                  alt: `Tip ${i + 1}`,
                  tipNumber:
                    parseInt(item.title.replace("Tip ", "")) ||
                    groupStart() + i + 1,
                }}
                on:click={() => {
                  selectedItem = item;
        ...
        <div
          class="slider_arrows white flex md:hidden justify-center w-full mt-8"
        >
          <div class="slider_arrow_left">
            <button
              onclick={handlePrevGroup}
      ...
      <div
        class="mt-6 text-xs font-paplane text-white opacity-80 text-center tracking-widest"
      >
...
<style>
  ...
  :global(.tip_content.para-box-wrapper .corner-square.tl) {
    top: -3px;
    left: -3px;
    border-top: 2px solid var(--color-primary);
    border-left: 2px solid var(--color-primary);
  }
  :global(.tip_content.para-box-wrapper .corner-square.tr) {
    top: -3px;
    right: -3px;
    border-top: 2px solid var(--color-primary);
    border-right: 2px solid var(--color-primary);
  }
  :global(.tip_content.para-box-wrapper .corner-square.bl) {
    bottom: -3px;
    left: -3px;
    border-bottom: 2px solid var(--color-primary);
    border-left: 2px solid var(--color-primary);
  }
  :global(.tip_content.para-box-wrapper .corner-square.br) {
    bottom: -3px;
    right: -3px;
    border-bottom: 2px solid var(--color-primary);
    border-right: 2px solid var(--color-primary);
