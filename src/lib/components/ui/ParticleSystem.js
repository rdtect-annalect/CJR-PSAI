...
class Particle {
  constructor(canvas, options = {}) {
    ...
    this.matrixChars = options.matrixChars || '01';
    this.currentChar = this.matrixChars[Math.floor(Math.random() * this.matrixChars.length)];
    this.charChangeTimer = Math.random() * 100;
  }
  ...
  update(mouseX, mouseY, connectionDist, visibilityDist) {
    ...
    if (this.matrixMode) {
      ...
      if (this.charChangeTimer <= 0) {
        this.currentChar = this.matrixChars[Math.floor(Math.random() * this.matrixChars.length)];
        this.charChangeTimer = Math.random() * 60 + 30; // Change every 0.5-1.5 seconds
      }
    }
  ...
  draw(ctx) {
    ...
    if (this.matrixMode) {
      ...
      ctx.font = `${this.size * 12}px 'Courier New', monospace`;
...
export class ParticleSystem extends BaseComponent {
  ...
  constructor(config = {}) {
    ...
    this.config = {
      ...
      matrixMode: false,           // Enable matrix-style effects
      matrixChars: '01010110100101', // Binary characters for matrix mode
  ...
  _generateParticles() {
    ...
    for (let i = 0; i < this.config.particleCount; i++) {
      this.particles.push(new Particle(this.canvas, { 
        ...
        matrixMode: this.config.matrixMode,
        matrixChars: this.config.matrixChars
      }));
    }
  }
