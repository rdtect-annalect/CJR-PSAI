...
export class FightingAIGallery extends BaseComponent {
  ...
  async init() {
    try {
      ...
      // Get main container
      this.container = DOM.select(this.config.containerSelector);
  ...
  async loadAllData() {
    try {
      const data = await Data.load(this.config.dataPath);
  ...
  setupEventListeners() {
    ...
    // Filter buttons
    const filterButtons = DOM.select(this.config.filterSelector, document, true);
