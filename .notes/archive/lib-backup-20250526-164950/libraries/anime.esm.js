...
const createDrawable = (selector, start = 0, end = 0) => {
  ...
  return els.map(($el) =>
    createDrawableProxy(/** @type {SVGGeometryElement} */ ($el), start, end)
  );
};
...
const parseEasings = (ease) =>
  isFnc(ease)
    ? ease
    : isStr(ease)
    ? parseEaseString(/** @type {String} */ (ease), eases, JSEasesLookups)
    : none;
...
const convertValueUnit = (el, decomposedValue, unit, force = false) => {
  ...
  if (!isUnd(cached) && !force) {
    ...
    if (currentUnit in angleUnitsMap) {
      convertedValue =
        (currentNumber * angleUnitsMap[currentUnit]) / angleUnitsMap[unit];
    } else {
      const baseline = 100;
          ...
          if (tween._renderTransforms) {
            if (!Object.keys(cachedTransforms).length) {
              ...
              for (let key in cachedTransforms) {
                str +=
                  transformsFragmentStrings[key] + cachedTransforms[key] + ") ";
              }
  ...
  constructor(targets, params) {
    ...
    const scroll =
      autoplay && /** @type {ScrollObserver} */ (autoplay).link
        ...
        : false;
    const alternate =
      params.alternate && /** @type {Boolean} */ (params.alternate) === true;
    const reversed =
      params.reversed && /** @type {Boolean} */ (params.reversed) === true;
  ...
  if (isUnd(unit)) {
    ...
    if (
      ...
      if (unit === false) {
        ...
        return `${round(convertedValue.n, globals.precision)}${
          convertedValue.u
        }`;
      }
    }
...
const parseDraggableFunctionParameter = (value, draggable) =>
  value && isFnc(value) ? /** @type {Function} */ (value)(draggable) : value;
   ...
   `;
    ...
    if (!$existingDebug) {
      $thresholds.style.cssText += `background:
       ${lineCSS("#FFFF")}${gradientOffset(half - 10)} / ${
        isHori ? "100px 100px" : "100px 100px"
      } ${repeat},
       ${lineCSS("#FFF8")}${gradientOffset(half - 10)} / ${
        isHori ? "10px 10px" : "10px 10px"
      } ${repeat};
     `;
    }
    $triggers.style.cssText = `${baseCSS(
      "relative",
      0,
      0,
    ...
    if (!$existingDebug) {
      $triggers.style.cssText += `background:
       ${lineCSS("#FFFF")}${gradientOffset(0)} / ${
        isHori ? "100px 10px" : "10px 100px"
      } ${repeat},
       ${lineCSS("#FFF8")}${gradientOffset(0)} / ${
